...
## 1.1 Introduction
Syncthing is an open‑source project that enables peer‑to‑peer file synchronization across devices. Rather than relying on a central server that stores your data, Syncthing routes traffic through public relay and discovery servers. Some of these servers are operated by the Syncthing team, while others are run by community contributors. Because the architecture is decentralized, your files are never stored on a single, centralized location they’re merely passed through the relays. The open‑source nature of the project also lets you host your own relay and discovery servers. You can choose to contribute those resources back to the community letting people to use your hosting Syncthing Relay & Discovery service, or keep them private for your exclusive use. Running your own relay/discovery service can be done on a **virtual private server (VPS)** or on **home server** that can be reacable from internet. In this guide, I’ll show you how to set up the servers using docker and configure the containers for optimal security.
...
To locate peers on the Internet, Syncthing use a discovery server. Anyone can run a discovery server and configure their Syncthing instances to use it. In addition, the Syncthing project operates a global, publicly‑available discovery server that all people can use it. [Here](https://relays.syncthing.net/) a list of all public relay.
...
# 2.0 Docker Installation & Configuration - Server-Side
## 2.1 Setting up Server-Side
Each container has its own isolated filesystem, network stack, and process space, typically dedicated to a single service. By running the service inside a container, you keep it sandboxed from the host system and from other applications, simplifying deployment, scaling, and security.
...
## 2.2 Installation Server-Side - docker-compose.yml
...
Now let's create the docker-compose.yml configuration:
```bash
Pv0t-VPS[/home/$USER/syncthing]$ vim docker-compose.yml
```

Here the configuration of the docker-compose.yml:
...
Inside the docker‑compose.yml file, update the two variables as follows:
...
Let's start our docker container and read the logs of the syncthing relay service.
```bash
...
# 3.0 Installation - Client-Side
Let’s proceed to install the GPG keys and add the Syncthing repository so we can install Syncthing on our distribution.
```bash
...
- Navigate to:  Actions -> Settings -> Connections.
...
- **`{$VPS_IP}`**: The public IP address of the VPS that you are running your Syncthing relay  &  discovery services.
...
After filling in these values, save the settings. Your client is now configured to use your self‑hosted/VPS as Syncthing relay and discovery.
